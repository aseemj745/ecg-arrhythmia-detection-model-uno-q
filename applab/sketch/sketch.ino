...
void update_motion_state() {
  ...
  // Which line applies depends on which side we are currently on - that
  // is what makes this a Schmitt trigger instead of a single threshold.
  float trigger_g = motion_detected ? MOTION_EXIT_THRESHOLD_G
                                     : MOTION_ENTER_THRESHOLD_G;

  if (deviation > trigger_g) {
    motion_over_count++;
  ...
  if (!motion_detected && motion_over_count >= MOTION_ENTER_DEBOUNCE) {
    motion_detected = true;
  } else if (motion_detected && motion_under_count >= MOTION_EXIT_DEBOUNCE) {
